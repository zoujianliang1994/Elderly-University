...
public class NoticeController extends BaseController {
    ...
    public ModelAndView goAdd() {
        ...
        message.setCreateTime(DateUtil.getDay());
        ...
        try {
            ...
            if (null != noticeFile && !noticeFile.isEmpty()){
                String url = FileUpload.fileUp(noticeFile, filePath,
                        noticeFile.getOriginalFilename().split("\\.")[0] + "("+DateUtil.getTime()+")");
    ...
    public ModelAndView goEdit() {
        ...
        try {
            ...
            if (StringUtils.isNotBlank(buinessId)){
                message = noticeService.findById(buinessId); // 根据ID读取
    ...
    public ModelAndView goDetail() {
        ...
        try {
            User user = this.getUserInfo();
            // 放入当前用户id
            pd.put("schoolName", user.getSchoolName());
            Message message = messageService.findById(pd.getString("id"));
            mv.addObject("message", message); // 放入视图容器
        } catch (Exception e) {
