...
public class StudentInfoController extends BaseController {
   ...
   * @throws Exception
   */
  @RequestMapping(value = "/goSel")
  public ModelAndView goSel(Page page) throws Exception {
    ModelAndView mv = this.getModelAndView();
    PageData pd = new PageData();
    try {
      pd = this.getPageData();
      ...
      if(pc.containsKey("c_id")){
        gre =pc.getString("c_id");
      }
      List<PageData> grades_list = null;
      String[] grades = {};
      if(!StringUtils.isEmpty(gre)){
        grades =  gre.split(",");
        grades_list = studentInfoService.getStuGradesDetail(grades);
      }
      //学员操作记录
      List<PageData> tuition_list = studentInfoService.getStuTuitionDetail(pc.getString("id"));
      //学籍记录
      List<PageData> status_list = studentInfoService.getStuStatusDetail(pc.getString("id"));

      mv.addObject("pd", pc);
      mv.addObject("grades_list", grades_list);
      mv.addObject("tuition_list", tuition_list);
      mv.addObject("status_list", status_list);
      mv.setViewName("edu/studentInfo/studentInfo_sel");
    } catch (Exception e) {
      e.printStackTrace();
      logger.error(e.toString(), e);
    }
