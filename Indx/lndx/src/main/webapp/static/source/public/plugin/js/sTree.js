    ...
    Stree.prototype = {
        ...
        create: function (options) {
            ...
            this.$dom = $('<div class="itemBox lev' + options.lev + '" lev="' + options.lev + '">' +
                ...
                '<div class="title">' +
                 options.title +
                '</div>' +
                '<div class="item-label item-cheaked" data-checked="1">' +
                '<i class="icon-gou iconfont checked" ></i>' +
                '</div>' +
                '</div>' +
        ...
        keyup:function () {
            ...
            this.$el.find("input").on("keyup",function(){
                var input, filter,dome
                input = that.$el.find("input");
                filter = input.val();
                dome = that.$el.find(".title");
                // 循环所有列表，查找匹配项
                for (i = 0; i < dome.length; i++) {
                    if (dome[i].innerHTML.indexOf(filter) > -1) {
                        dome[i].style.display = "block";
                    } else {
        ...
        active: function (mark) {
            ...
            this.$el.on("click", ".item-cheaked", function () {
                ...
                if (isActive) {
                    ...
                    if (that.callback) that.callback(that.arr);
        ...
        switchIcon: function () {
            var that = this;
            this.$el.on("click", ".item-switch", showmuen);
            function showmuen() {
                var lev = $(this).closest(".itemBox").attr("lev");
