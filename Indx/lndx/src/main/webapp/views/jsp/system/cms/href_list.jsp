...
<div class="tableBox">
    <div class="operationBox">
        ...
        <form action="list.do" method="post" class="layui-form" name="Form" id="Form">
            <div class=" fl ">
                <input id="keywords" name="keywords" type="text" value="${pd.keywords}" placeholder="请输入链接名称" class="layui-input fl search layui-input-color">
                        ...
                        <c:forEach items="${varList}" var="var" varStatus="vs">
                            <tr>
                                ...
                                <td style="text-align: center;border-left: 1px solid #dce3eb;">
                                        ...
                                        <a class="table-btn bianji-btn edit" onclick="edit('${var.ID}','${var.NAME}','${var.URL}','${var.FLAG}','2');">
...
<script type="text/javascript">
    ...
    $("#add").on("click", function () {
        ...
        parent.parent.layer.open({
            title: "新增",
            type: 2,
            content: '<%=basePath%>manage/indexHref/addHref.do?currentPage=' + ${page.currentPage},
            area: ["700px", "500px"]
        });
    });

    function edit(id,name,url,flag,type) {
        ...
        parent.parent.layer.open({
            title: msg,
            type: 2,
            content: '<%=basePath%>manage/indexHref/toUpdate?ID='+id+'&NAME='+name+'&URL='+url+'&FLAG='+flag+'&type='+type,
            area: area
        });
    }

    function del(id) {
        parent.parent.layer.confirm("是否删除此链接?", {
            btn: ['确定', '取消'],
            resize: false,
            title: "提示",
            maxWidth: 800
        }, function (index) {
            var url = "<%=basePath%>manage/indexHref/delHref.do?ID=" + id;
            $.get(url, function (data) {
                if ("success" == data.result) {
                    window.location.href = "<%=basePath%>manage/indexHref/list.do?currentPage=${page.currentPage}";
