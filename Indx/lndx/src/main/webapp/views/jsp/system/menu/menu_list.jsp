					...
					<c:when test="${not empty menuList}">
						<c:forEach items="${menuList}" var="menu" varStatus="">
							<tr>
								...
								<td  style="width: 240px; text-align: center;border-left: 1px solid #dce3eb;">

									<shiro:hasPermission name="system:menu:permission_add">
										<c:if test="${menu.MENU_TYPE == '1' }">
											<a data_id="${menu.MENU_ID }" class="table-btn-max quanxiantianjia-btn addPermission">
												<%--<i class="iconfont icon-quanxiantianjia" title="权限添加"></i>--%>
												添加权限
											</a>
										</c:if>
									</shiro:hasPermission>
									...
									<shiro:hasPermission name="system:menu:edit">
										...
										<%--<a data_id="${menu.MENU_ID }" class="table-btn bianji-btn edit" href="javascript:editTb('${menu.MENU_ID }');">--%>
												<%--&lt;%&ndash;<i class="iconfont icon-bianji" title="编辑"></i>&ndash;%&gt;--%>
											<%--编辑图标--%>
										<%--</a>--%>

										<a data_id="${menu.MENU_ID }" class="table-btn bianji-btn edit" href="javascript:editmenu('${menu.MENU_ID }');">
									...
									<shiro:hasPermission name="system:menu:del">
										<a class="table-btn shanchu-btn del" data_id="${menu.MENU_ID }">
	...
	<script type="text/javascript">
        ...
        layui.use('laypage', function() {
			...
			$("#add").on("click", function() {
				 ...
				 parent.layer.open({
					  title:"<i  class=\"icon-add-size iconfont icon-add \"></i>\n" +
                      "<span>新增</span>",
					  type: 2,
