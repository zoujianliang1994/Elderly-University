...
<div class="menu-list">
	...
	<div class="layui-col-xs layui-col-sm layui-col-md layui-col-lg tableBox operationBox">
		...
		<form action="list.do" method="post" name="Form" id="Form" class="layui-form">
			...
			<div class=" fl ">
				<input id="keywords" name="keywords" type="text" value="${pd.keywords}" placeholder="请输入教室名称" class="layui-input fl search layui-input-color">
		...
		<!-- table -->
			<table class="layui-table center " lay-skin="line" id="table">
				...
				<tr style="background-color: #f3f8ff;">
					...
					<th class='center'>教室容量(人)</th>
					<th class='center'>教室位置</th>
					<th class='center'>所属校区/教学点</th>
					<th class='center'>使用情况(小时/周)</th>
				...
				<c:choose>
					<c:when test="${not empty classroom_List}">
						<shiro:hasPermission name="edu:classroom:sel">
						<c:forEach items="${classroom_List}" var="room" varStatus="vs">
							<tr>
								...
								<td> <a style="cursor: pointer;" class="use" data_id="${room.id }" >${room.weekTime}小时</a> </td>
								<td  style="width: 200px; text-align: center;border-left: 1px solid #dce3eb;">

									<a data_id="${room.id }" class="table-btn chakan-btn sel">
					...
					<shiro:lacksPermission name="edu:classroom:sel">
						<tr>
