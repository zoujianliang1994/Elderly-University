...
public class UserService implements UserManager {
	...
	public PageData initeAccount(PageData pd) throws Exception {
		...
		String password = StringUtils.isEmpty(pd.getString("password")) ? "123456" : pd.getString("password");// 初始密码
		...
		PageData userPd = this.findByUsername(pd);
		if (null != userPd && userPd.size() > 0) {
			...
			saveU(pd); // 执行保存
			resultPd.put("userId", userId);
			resultPd.put("userName", userName);
			resultPd.put("password", password);
		}
	...
	@Override
	public int validateUserName(PageData pd) throws Exception {
		if (PhoneUtils.isMobileNO(pd.getString("USERNAME"))) {
			// 短信验证
			return SmsUtil.sendSmsInfo(pd.getString("USERNAME"), PropertyUtils.getValue("SMS_TEMPLATE_01"), new HashMap<String, String>());
		} else {
			// 邮箱验证
			pd.put("title", "系统通知");
