(function($){
    ...
    TreeView.prototype= {
        ...
        create:function(options){
            ...
            //     icon= "icon-quan"
            // }
            html='<div class="itemBox lev'+options.lev+'" lev="'+options.lev+'">'+
                    '<div class="treeItem '+(options.lev == 1 ? "active" : "")+'">'+
                        '<i class="iconfont '+options.icon+' icon-icon"></i>'+
                        '<span class="title" title="'+options.title+'">'+options.title+'</span>';
            if(options.lev.length>2){
              html+=  '<i  class="item-switch iconfont icon-xiala1-copy " style="'+(options.children=="" && !options.hasNext?"display:none":"")+'"></i>'+
                  '</div>'+
                ...
                html+= '</div>'+
                '<div class="itemBox" style="display:block">'+
                '</div>'+
                '</div> ';
            }
            this.$dom=$(html)
