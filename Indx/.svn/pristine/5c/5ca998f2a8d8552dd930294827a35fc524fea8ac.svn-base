...
public class TeachPlanController extends BaseController {
  ...
  public Object save(@RequestParam(value = "file1", required = false) MultipartFile file1, HttpServletRequest request) {
    ...
    try {
      ...
      pd.put("create_time", DateUtil.fomatDate(DateUtil.getTime()));
      pd.put("teacher_id", getUserInfo().getUSER_ID());
      //附件
  ...
  public ModelAndView goEditU() {
    ...
    try {
      ...
      if ("0".equals(sh_type)) {
        pd.put("shText", "草稿");
      } else if ("1".equals(sh_type)) {
        pd.put("shText", "等待审核");
      } else if ("2".equals(sh_type)) {
        pd.put("shText", "审核通过");
      } else {
        pd.put("shText", "已驳回");
      }
  ...
  public void downExcel(HttpServletResponse response) {
    PageData pd = getPageData();
    String file_url = pd.getString("file_url");
    try {
      FileDownload.fileDownload(response, PathUtil.getClasspath() + Const.FILEPATHFILE + file_url, file_url);
    } catch (Exception e) {
