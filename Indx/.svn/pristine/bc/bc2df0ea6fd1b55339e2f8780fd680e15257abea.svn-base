            ...
            <div class="page-content">
                <div class="row">
                    <div class="col-xs-12">
                        ...
                        <!-- 检索  -->
                        <form action="manage/cover/list.do" method="post" name="Form" id="Form">
...
<script type="text/javascript">
    ...
    function add() {
        ...
        diag.Drag = true;
        diag.Title = "新增";
        diag.URL = "<%=basePath%>manage/cover/goAdd.do";
        diag.Width = 660;
        diag.Height = 550;
        diag.CancelEvent = function () { //关闭事件
            if ('none' == diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display) {
                window.location.href = "<%=basePath%>manage/cover/list.do?dnowPage=${page.currentPage}";
            }
            diag.close();
        };
        diag.show();
    ...
    function edit(ID) {
        ...
        diag.Drag = true;
        diag.Title = "编辑";
        diag.URL = '<%=basePath%>manage/cover/goEdit.do?ID=' + ID;
        diag.Width = 660;
        diag.Height = 550;
        diag.CancelEvent = function () { //关闭事件
    ...
    function del(ID) {
        bootbox.confirm("确定要删除吗?", function (result) {
            if (result) {
                top.jzts();
                var url = "<%=basePath%>manage/cover/delete.do?ID=" + ID;
