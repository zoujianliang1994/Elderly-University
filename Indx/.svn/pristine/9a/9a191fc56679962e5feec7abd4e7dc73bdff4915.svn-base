...
public class TeacherController extends BaseController {
  ...
  public ModelAndView list(Page page) throws Exception {
    ...
    try {
      ...
      pd.put("school_id", teacherService.findBySchoolPid(getUserInfo().getSchoolId().split(",")[0]).getString("pid"));
  ...
  public Object save(@RequestParam(value = "file1", required = false) MultipartFile file1, HttpServletRequest request) {
    ...
    try {
      ...
      PageData schoolPd = teacherService.findBySchoolPid(getUserInfo().getSchoolId().split(",")[0]);
      pd.put("school_id", schoolPd.getString("pid"));
      pd.put("school_name", schoolPd.getString("school_name"));
      //通过身份证+学校去查询老师是否重复添加
      if (getReptTeacherListSize(pd) == 0) {
        ...
        PageData pdUser = insertIntoTeacherInfo(pd);
        pdUser.put("pwdStrategy", "2");
        pdUser.put("password", "123456");
        //执行保存教师账号
        PageData returnPd = userService.initeAccount(pdUser);
        String returnMsg = returnPd.getString("msg");
        if ("hasaccount".equals(returnMsg)) {
          msg = "hasaccount";
        } else {
          pd.put("create_id", getUserInfo().getUSER_ID());
          // 执行保存教师档案
  ...
  public ModelAndView goEditU() {
    ...
    try {
      ...
      if ("1".equals(type)) {
        //查询教师履历信息
        List<PageData> varList = teacherService.teacherLvList(pd);
        mv.addObject("varList", varList);
      }
      pd.put("type", type);
  ...
  public Object editU(@RequestParam(value = "file1", required = false) MultipartFile file1, HttpServletRequest request) {
    ...
    try {
      ...
      String tel = pd.getString("tel").replace(" ", "");
      String old_tel = pd.getString("old_tel").replace(" ", "");
      ...
      if (idcard.equals(old_idcard) || (!idcard.equals(old_idcard) && getReptTeacherListSize(pd) == 0)) {
        ...
        pd.put("USERNAME", tel);
        PageData pdCount = userService.findByUsername(pd);
        if (old_tel.equals(tel) || (!tel.equals(old_tel) && null == pdCount)) {
          PageData schoolPd = teacherService.findBySchoolPid(getUserInfo().getSchoolId().split(",")[0]);
          pd.put("school_id", schoolPd.getString("pid"));
          pd.put("school_name", schoolPd.getString("school_name"));
          PageData pdUser = insertIntoTeacherInfo(pd);
          //执行修改教师账号
          userService.editU(pdUser);
          //执行修改教师档案
  ...
  private PageData insertIntoTeacherInfo(PageData pd) {
    //执行保存教师账号
    PageData pdUser = new PageData();
    pdUser.put("USER_ID", pd.getString("id"));
    pdUser.put("ROLE_ID", PropertyUtils.getValue("TEACHER_ROLE_ID"));
    pdUser.put("USERNAME", pd.getString("tel"));
    pdUser.put("NAME", pd.getString("name"));
    pdUser.put("SCHOOL_ID", pd.getString("school_id"));
    pdUser.put("SCHOOL_NAME", pd.getString("school_name"));
    pdUser.put("phone", pd.getString("tel"));
    pdUser.put("PHONE", pd.getString("tel"));
    pdUser.put("SEX", pd.getString("xb"));
    pdUser.put("MZ", pd.getString("mz"));
    pdUser.put("TYPE", "3");
