...
public class DateUtil {
  ...
  /**
   * nowDate不为空默认当前日期 当月第一天日期(t1=0,t2=1) 当月最后一天日期(t1=1,t2=0)
   */
  ...
  /**
   * 计算两个日期相差的月份数 date1 日期1 date2 日期2 pattern 日期1和日期2的日期格式 return 相差的月份数
   */
   ...
   * @param timeParts  时间区间组
   * @return
   * @throws ParseException
   */
  public static boolean isEffectiveDate(String nowTimeStr, String[] timeParts) throws ParseException {
    String format = "HH:mm:ss";
    boolean result = true;
    Date nowTime = new SimpleDateFormat(format).parse(nowTimeStr);
    for (int i = 0; i < timeParts.length; i++) {
      String[] time = timeParts[i].split("-");
      Date startTime = new SimpleDateFormat(format).parse(time[0]);
      Date endTime = new SimpleDateFormat(format).parse(time[1]);
