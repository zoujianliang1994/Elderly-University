package com.zhimu.commons.utils.mail;
...
public class SimpleMailSender {
	...
	public static void main(String[] args) throws Exception {
		...
		mailInfo.setMailServerPort("25");
		mailInfo.setValidate(true);
		mailInfo.setUserName("service@leholand.com");
		mailInfo.setPassword("Lol0021");// 您的邮箱密码
		mailInfo.setFromAddress("service@leholand.com");
		mailInfo.setToAddress("986073546@qq.com");
	...
	public static void sendMessageToSchool(PageData pd) {
		// 这个类主要是设置邮件
		MailSenderInfo mailInfo = new MailSenderInfo();
		mailInfo.setMailServerHost(PropertyUtils.getValue("MAIL_SERVER_HOST"));
		mailInfo.setMailServerPort(PropertyUtils.getValue("MAIL_SERVER_PORT"));
		mailInfo.setValidate(true);
		mailInfo.setUserName(PropertyUtils.getValue("MAIL_SERVER_USERNAME"));
		mailInfo.setPassword(PropertyUtils.getValue("MAIL_SERVER_PASSWORD"));
		mailInfo.setFromAddress(PropertyUtils.getValue("MAIL_SERVER_USERNAME"));
		mailInfo.setToAddress(pd.getString("toAddress"));
		mailInfo.setSubject(pd.getString("title"));
		mailInfo.setContent(pd.getString("content"));
		// 这个类主要来发送邮件
		SimpleMailSender sms = new SimpleMailSender();
		// sms.sendTextMail(mailInfo);// 发送文体格式
		try {
